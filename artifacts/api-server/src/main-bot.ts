...
if (!BOT_TOKEN) {
  logger.warn('TELEGRAM_BOT_TOKEN is not set — main bot will be disabled');
}

export const mainBot = BOT_TOKEN ? new Telegraf<Context>(BOT_TOKEN) : null;
...
mainBot?.on('text', async (ctx) => {
  ...
  if (state.mode === 'deposit') {
    ...
    if (isNaN(amount) || amount < 10) { await ctx.reply('❌ Minimum 10 ETB.'); return; }
    await ctx.reply(
  ...
  if (state.mode === 'withdraw') {
    clearState(userId);
    const amount = parseAmount(text);
    if (isNaN(amount) || amount < 50) { await ctx.reply('❌ Minimum 50 ETB.'); return; }
    ...
    try {
      ...
      const data: any = await response.json();
      if (response.ok) {
        await ctx.reply(`✅ Withdrawal request for ${amount} ETB received. Admin has been notified.`);
      } else {
        await ctx.reply(`❌ Request failed: ${data.error || 'Server error'}`);
      }
    } catch (err) {
      logger.error('Withdrawal request error', { error: String(err), userId });
  ...
  if (replyText.includes('DEPOSIT INSTRUCTIONS')) {
    ...
    await ctx.reply('✅ Confirmation received! Admin is verifying.');
  }
});
...
mainBot?.catch((err) => {
  ...
  logger.error('Main Bot Global Error', { error: String(err) });
});