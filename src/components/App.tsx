...
export default function App() {
  const [phase, setPhase] = useState<AppPhase>(() => {
    ...
    return saved && VALID_PHASES.includes(saved as AppPhase)
      ? (saved as AppPhase)
      : 'home';
  });
  ...
  const [roomStats, setRoomStats] = useState<
    RoomStats & { selectionTimeLeft?: number }
  >({
    pool: 0,
    players: 0,
    gameId: '---',
  ...
  useEffect(() => {
    ...
    const handleStatus = (status: {
      ...
      if (status.referredCount !== undefined)
        setReferredCount(status.referredCount);
    };
  ...
  const handleTabChange = useCallback(
    (tab: BottomTabKey) => {
      setBottomTab(tab);
      if (tab === 'game') {
        if (phase !== 'selection' && phase !== 'game') setPhase('home');
      ...
      >
        ...
        {showLoader && (
          <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#0f170a]">
            <RefreshCw
              size={32}
              className="text-yellow-500/50 animate-spin mb-4"
            />
        ...
        {showTelegramError && (
          <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#0f170a] p-8 text-center">
            ...
            <p className="text-gray-400 text-sm mb-8 max-w-xs leading-relaxed">
              This app must be opened through the Telegram bot. Please use the
              bot link to access Lomi Bingo.
            </p>
            <button
        ...
        {showVerifyScreen && (
          <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-[#1a1b2e] p-8 text-center">
            ...
            <p className="text-gray-400 text-sm mb-8 max-w-xs leading-relaxed">
              To ensure secure payments and fair play, please share your phone
              number with our bot.
            </p>
            <button
        ...
        {isVerified === true && !connectionError && (
          <>
            {isMaintenanceMode && (
              <div className="bg-red-600 text-white px-4 py-2 z-[202] flex items-center justify-center gap-2 shadow-lg shrink-0">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-center">
                  Maintenance Mode: System paused. New bets are currently
                  disabled.
                </span>
              </div>
            )}
                ...
                {phase === 'home' && (
                  ...
                  >
                    <Dashboard
                      ...
                      isPlayDisabled={
                        isMaintenanceMode || !roomStats.isEngineActive
                      }
                      playButtonLabel={getPlayLabel(
                        roomStats,
                        isMaintenanceMode
                      )}
                  ...
                  >
                    <ProfilePage
                      ...
                      totalEarnings={history.reduce(
                        (sum, h) => (h.isMyWin ? sum + h.payoutPerWinner : sum),
                        0
                      )}
                      telegramDisplayName={telegramDisplayName}
            ...
            {phase !== 'game' && (
              <BottomTabs active={bottomTab} onTabChange={handleTabChange} />
            )}
              ...
              {showGameStoppedModal && (
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() => setShowGameStoppedModal(null)}
                      className="px-6 py-3 bg-black text-white rounded-2xl font-black text-xs uppercase tracking-widest"
                    >
                      Got it
                    </button>
                  </motion.div>
              ...
              {showEngineIdleModal && (
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-gray-500 text-sm font-medium leading-relaxed mb-6">
                      The admin is getting things ready. Please wait a moment
                ...
                >
                  ...
                  >
                    ...
                    <div className="space-y-6">
                      <RuleItem
                        number="1"
                        text={`Stake ${SINGLE_STAKE} ETB to enter the round.`}
                      />
                      <RuleItem
                        number="2"
                        text="Pick your board from the 600 available options within 40 seconds."
                      />
                      <RuleItem
                        number="3"
                        text="Wait for the system to call a ball every 3 seconds."
                      />
                      <RuleItem
                        number="4"
                        text="Numbers are marked automatically. Complete a row, column, diagonal, or four corners to win."
                      />
                      <RuleItem
                        number="5"
                        text="After a win, you have 10 seconds to view results, then pick a board for the next round."
                      />
                    </div>
                    <button
                      onClick={() => setShowRules(false)}
