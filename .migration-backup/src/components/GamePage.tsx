...
import { BingoBoardData, HistoryEntry, PoolUpdateData } from '../types';
...
const WinnerCard = memo(
  ({
    ...
    const winningIndices = useMemo(
      () =>
        new Set(
          winner.patterns.flatMap((p: WinningPattern) =>
            p.indices.map((i) => `${i.r}-${i.c}`)
          )
        ),
      [winner.patterns]
    );
    ...
    return (
      <div
        key={winner.id}
        className={`bg-white/5 ${isCompact ? 'p-2' : 'p-3'} rounded-2xl border ${isMyBoard ? 'border-yellow-400 ring-2 ring-yellow-400/50' : 'border-white/5'}`}
      >
        <div
          className={`flex justify-between items-center ${isCompact ? 'mb-1' : 'mb-2'}`}
        >
          <div className="flex flex-col">
            <span
              className={`font-black ${isCompact ? 'text-[10px]' : 'text-xs'} uppercase tracking-tight leading-none ${isMyBoard ? 'text-yellow-400' : 'text-indigo-400'}`}
            >
              {isMyBoard
                ? `${t.boardNum}${winner.id} (YOU)`
            ...
            <div className="flex flex-wrap gap-1 mt-0.5">
              {winner.patterns.map((p: WinningPattern, pIdx: number) => (
                <span
                  key={pIdx}
                  className={`${isCompact ? 'text-[6px]' : 'text-[7px]'} font-black bg-yellow-400/20 text-yellow-400 px-1 py-0.5 rounded uppercase`}
                >
                  {p.name}
                </span>
              ))}
          ...
          <span
            className={`text-green-400 ${isCompact ? 'text-sm' : 'text-lg'} font-black italic`}
          >
            {((totalPrize || 0) / (winnersCount || 1)).toFixed(0)} ETB
          </span>
        </div>
        <div className="grid grid-cols-5 gap-0.5">
          {Array.isArray(winner.grid) &&
            winner.grid.map((row: BingoBoardData[number], rIdx: number) =>
              row.map(
                (
                  cell: { value: number | 'FREE'; marked: boolean },
                  cIdx: number
                ) => {
                  const isMarkedWinner =
                  ...
                  return (
                    <div
                      key={`${rIdx}-${cIdx}`}
                      className={`aspect-square flex items-center justify-center ${isCompact ? 'text-[7px]' : 'text-[8px]'} font-bold rounded-sm border ${
                        isWinningCell
  ...
  ({
    ...
    return (
      <button
        ...
        className={`aspect-square flex items-center justify-center text-[12px] font-black rounded-md transition-all ${
          isCurrentBall
            ? 'bg-orange-500 text-white shadow-[0_0_15px_rgba(249,115,22,0.6)] z-10 scale-110'
            : isMarked
...
export default function GamePage({
  ...
  const [isMuted, setIsMuted] = useState(
    () => localStorage.getItem('bingo_muted') === 'true'
  );
  ...
  useEffect(() => {
    if (autoMarkMode && calledNumbers.size > 0) {
  ...
  useEffect(() => {
    ...
    if (popupTimeLeft > 0) return;

    onRestartGame();
  }, [popupTimeLeft, showWinnerPopup, onRestartGame]);
  ...
  useEffect(() => {
    ...
    const handleInit = (data: {
      ...
      const initialBalls = new Set(data.balls);
      ...
      if (data.balls.length > 0)
        setCurrentBall(data.balls[data.balls.length - 1]);
    };
    ...
    const handleReset = () => {
      if (winnersRef.current.length > 0) {
        const isMyWin = winnersRef.current.some((w) =>
          selectedBoardIdsRef.current.includes(w.id)
        );
        onGameEnd({
          ...
          payoutPerWinner:
            winnersRef.current.length > 0
              ? gameMetadataRef.current.pool / winnersRef.current.length
              : 0,
          isMyWin: isMyWin,
        });
      }
    ...
    const handleServerWinner = (
      winnerData: WinnerData & { gameId: string; time: string }
    ) => {
      setWinners((prev) => {
        const boardId = winnerData.boardId;
        if (prev.some((w) => w.id === boardId)) return prev;
    ...
    const handleCountdown = (seconds: number) => {
      if (showWinnerPopupRef.current) setPopupTimeLeft(seconds);
    };
    ...
    () => (
      <div className="grid grid-cols-5 gap-1 mb-1">
        {['B', 'I', 'N', 'G', 'O'].map((l, i) => (
          <div
            key={l}
            className={`text-center text-[12px] font-black py-1.5 rounded-sm ${
              i === 0
                ? 'bg-blue-500'
                : i === 1
                  ? 'bg-indigo-600'
    ...
    <div className="flex-1 flex flex-col bg-primary text-white overflow-hidden select-none">
          ...
          <div className="flex items-center justify-between p-1">
            <div className="flex gap-1">
              {historyBalls.map((n) => (
                <div
                  key={n}
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-[9px] font-black border-2 opacity-60 ${getBallColor(n)}`}
                >
                  {getLetter(n)}-{n}
                </div>
              ))}
            ...
            <div className="h-24 bg-[#23243d] rounded-xl border border-white/10 flex items-center justify-center relative overflow-hidden">
              ...
              <div className="w-16 h-16 rounded-full bg-white border-4 border-lime-400 flex items-center justify-center shadow-[0_0_20px_rgba(250,204,21,0.5)] z-10">
                <span className="text-xl font-black text-indigo-950">
                  {currentBall
                    ? `${getLetter(currentBall)}-${currentBall}`
                    : '--'}
                </span>
              </div>
            </div>
          ...
          <div className="flex-1 bg-[#23243d] rounded-xl border border-white/10 overflow-hidden flex flex-col">
            {selectedBoardIds.length === 0 ? (
              ...
              <div className="flex-1 overflow-hidden p-1 flex flex-col justify-center">
                {boardsData
                  .slice(0, 1)
                  .map(({ id, grid }: { id: number; grid: BingoBoardData }) => (
                    <div
                      key={id}
                      className="p-3 bg-indigo-900/60 rounded-2xl border border-white/10 shadow-2xl mx-1"
                    >
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-[11px] font-black text-indigo-300 uppercase tracking-tighter">
                          {t.boardNum}
                          {id}
                        </span>
                      </div>
                      <div className="grid grid-cols-5 gap-1">
                        {grid.map((row, rIdx: number) =>
                          row.map((cell, cIdx: number) => {
                            const isMarkedLocal =
                              typeof cell.value === 'number'
                                ? manualMarks.has(cell.value)
                                : cell.value === 'FREE';
                            const isCurrentBall =
                              typeof cell.value === 'number' &&
                              cell.value === currentBall;
                            const isPendingMark =
                              !autoMarkMode &&
                              typeof cell.value === 'number' &&
                              calledNumbers.has(cell.value) &&
      ...
      <div className="p-2 grid grid-cols-4 gap-2 bg-[#2d2e4d] border-t border-white/10">
        ...
        >
          <RefreshCw
            size={16}
            className={`text-lime-400 ${isResyncing ? 'animate-spin' : ''}`}
          />
          <span className="text-[8px] font-black text-white uppercase">
            {t.resync}
          </span>
        </button>
        <div className="col-span-2 h-14 rounded-xl bg-[#b19539] text-indigo-950 font-black text-sm italic uppercase tracking-tighter shadow-inner px-4 flex items-center justify-center text-center">
          {footerStatus}
        </div>
      </div>
          ...
          >
            ...
            >
              <div className="bg-indigo-600 p-4 text-center">
                <Trophy
                  className="text-yellow-400 w-8 h-8 mx-auto mb-1"
                  aria-hidden="true"
                />
                <h2
  ...
  ({ label, value }: { label: string; value: string | number }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center bg-white/5 rounded-xl py-2 border border-white/5">
        <span className="text-[9px] text-gray-400 font-black uppercase tracking-tight leading-none mb-0.5">
          {label}
        </span>
        <span className="text-[11px] font-black italic leading-none text-lime-400">
          {value}
        </span>
      </div>
    );
...
const RegisterCell = memo(
  ({
    ...
    RegisterCell.displayName = 'RegisterCell';
    return (
      <div
        className={`flex items-center justify-center text-[11px] font-bold rounded-sm border border-white/5 ${
          isCalled
            ? colIndex === 4
