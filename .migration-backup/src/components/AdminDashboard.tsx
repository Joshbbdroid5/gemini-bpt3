...
export default function AdminDashboard({ onBack }: Props) {
  ...
  const [wallets, setWallets] = useState<
    Record<string, { balance: number; username?: string }>
  >({});
  ...
  const [roundSort, setRoundSort] = useState<{
    key: 'date' | 'players' | 'pool';
    order: 'asc' | 'desc';
  }>({ key: 'date', order: 'desc' });
  ...
  const fetchWallets = async () => {
    ...
    try {
      ...
      if (!healthResp.ok) {
        throw new Error(
          `Health check failed: ${healthResp.status} ${healthResp.statusText}`
        );
      }
  ...
  }, [isAuthenticated, secret, backendUrl, fetchWallets]);

  const triggerUpdateBalance = (
    userId: string,
    amount: number,
    type: 'add' | 'subtract' | 'set'
  ) => {
  ...
  const fetchUserActivity = async (userId: string) => {
    ...
    try {
      const response = await fetch(
        `${backendUrl}/admin/user-activity?userId=${userId}&secret=${secret}`
      );
      if (response.ok) {
  ...
  const confirmUpdateBalance = async () => {
    ...
    try {
      const finalAmount =
        type === 'add' ? amount : type === 'subtract' ? -amount : amount;
  ...
  const handleToggleMaintenance = async () => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to ${nextState ? 'ENABLE' : 'DISABLE'} maintenance mode?`
      )
    )
      return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          `Maintenance mode ${data.isMaintenanceMode ? 'enabled' : 'disabled'}`
        );
      } else {
  ...
  const filteredWallets = Object.entries(wallets).filter(
    ([id, data]) =>
  ...
  const processedRounds = useMemo(() => {
    ...
    if (roundSearch) {
      result = result.filter((r) =>
        r.gameId?.toLowerCase().includes(roundSearch.toLowerCase())
      );
    }
    ...
    if (dateRange.start) {
      ...
      result = result.filter(
        (r) => new Date(r.date || r.timestamp || 0).getTime() >= startTime
      );
    }
    ...
    if (dateRange.end) {
      ...
      result = result.filter(
        (r) => new Date(r.date || r.timestamp || 0).getTime() <= endTime
      );
    }
  ...
  const exportRoundsToCSV = () => {
    ...
    const rows = processedRounds.map((round) => {
      const winnersStr =
        round.winners
          ?.map((w: any) => `#${w.boardId}(${w.payout.toFixed(0)})`)
          .join('; ') || '';
      const ballsStr = (round.ballsDrawn || []).join(' ');
      const staked =
        round.totalStaked || (round.players ? round.players * 10 : 0);
    ...
    link.setAttribute(
      'download',
      `rounds_history_${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 bg-primary">
        ...
        <h2 className="text-3xl font-black text-white uppercase italic mb-8 text-center tracking-tight">
          Admin Access
        </h2>
        <input
          type="password"
        ...
        <button
          onClick={onBack}
          className="mt-4 text-gray-500 font-bold uppercase text-[10px]"
        >
          Go Back
        </button>
      </div>
  ...
  return (
    <>
          ...
          <div className="w-full lg:w-[400px] border-r border-white/5 flex flex-col overflow-y-auto custom-scrollbar bg-black/10">
            <div className="p-4 space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="px-2 text-[10px] font-black text-indigo-300 uppercase tracking-[0.2em]">
                  System Controls
                </h3>
                <div className="bg-black/20 rounded-[32px] p-2 border border-white/5 shadow-inner space-y-2">
                  {/* Maintenance Toggle */}
                  <div
                    className={`p-4 rounded-[24px] border flex items-center justify-between transition-all ${
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2.5 rounded-2xl ${stats.isMaintenanceMode ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}
                      >
                        <Power size={20} />
                      </div>
                      <div>
                        <h3 className="text-white font-black uppercase text-xs tracking-widest leading-none mb-1">
                          Maintenance
                        </h3>
                        <p className="text-[9px] text-gray-400 uppercase font-bold">
                          {stats.isMaintenanceMode ? 'Paused' : 'Active'}
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="p-4 rounded-[24px] border border-white/5 bg-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2.5 rounded-2xl ${stats.isEngineActive ? 'bg-indigo-500/20 text-indigo-400' : 'bg-gray-500/20 text-gray-400'}`}
                      >
                        <Activity size={20} />
                      </div>
                      <div>
                        ...
                        <p className="text-[9px] text-gray-400 uppercase font-bold">
                          {stats.isEngineActive ? 'Running' : 'Stopped'}
                        </p>
                      </div>
                    </div>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      const response = await fetch(
                        `${backendUrl}/admin/force-start-round`,
                        {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ secret }),
                      ...
                      if (response.ok) {
                        ...
                        toast.error(
                          data.error || 'Failed to force-start round'
                        );
                      }
                    } catch {
                      toast.error('Connection error');
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-indigo-500/5 border border-white/5 rounded-2xl p-3 shadow-sm">
                    ...
                    <div className="text-lg font-black text-white italic">
                      {stats.stakes24h.toFixed(0)}{' '}
                      <span className="text-[10px] not-italic text-gray-500">
                        ETB
                      </span>
                    </div>
                  </div>
                  <div className="bg-green-500/5 border border-white/5 rounded-2xl p-3 shadow-sm">
                    <span className="text-[9px] font-black text-green-400 uppercase tracking-widest block mb-1">
                      24h Total Payouts
                    </span>
                    <div className="text-lg font-black text-white italic">
                      {stats.payouts24h.toFixed(0)}{' '}
                      <span className="text-[10px] not-italic text-gray-500">
                        ETB
                      </span>
                    </div>
              ...
              <AnimatePresence mode="wait">
                {activeTab === 'wallets' ? (
                  ...
                  >
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-2 sticky top-0 z-10 bg-primary/80 backdrop-blur-md py-2">
                          <div className="bg-indigo-600/20 border border-indigo-500/30 rounded-2xl p-4 shadow-md">
                            ...
                            <div className="text-xl font-black text-white italic">
                              {roundsSummary.totalVolume.toFixed(0)}{' '}
                              <span className="text-xs not-italic text-indigo-400 ml-1">
                                ETB
                              </span>
                            </div>
                          </div>
                          <div className="bg-green-600/20 border border-green-500/30 rounded-2xl p-4 shadow-md">
                            ...
                            <div className="text-xl font-black text-white italic">
                              {roundsSummary.totalProfit.toFixed(0)}{' '}
                              <span className="text-xs not-italic text-green-400 ml-1">
                                ETB
                              </span>
                            </div>
                          </div>
                    ...
                    <div className="space-y-3 mb-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="relative">
                          ...
                          />
                          {!dateRange.start && (
                            <span className="absolute left-9 top-1/2 -translate-y-1/2 text-gray-500 text-[9px] font-black uppercase tracking-widest pointer-events-none">
                              From
                            </span>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
                        <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest whitespace-nowrap mr-1">
                          Sort:
                        </span>
                        <RoundSortBtn
                          label="Date"
                          activeKey="date"
                          currentSort={roundSort}
                          onSort={setRoundSort}
                        />
                        <RoundSortBtn
                          label="Players"
                    ...
                    {processedRounds.length === 0 ? (
                      <div
                        className="flex flex-col items-center justify-center py-20 opacity-20 border-2 border-dashed border-white/10 rounded-[32px]"
                        aria-hidden="true"
                      >
                        <History size={48} className="text-white mb-4" />
                        <p className="font-black text-white uppercase text-xs tracking-widest">
                  ...
                  >
                    <div className="flex items-center justify-between px-2 mb-2 sticky top-0 z-10 bg-primary/80 backdrop-blur-md py-4">
                      ...
                      <span className="text-[9px] font-bold text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded-full italic">
                        Approved & Requested
                      </span>
                    </div>
                    ...
                    {recentActivity.length === 0 ? (
                      ...
                      recentActivity.map((log, i) => (
                        <TransactionCard key={i} log={log} wallets={wallets} />
                      ))
                    )}
                  </motion.div>
                )}
      ...
      <AnimatePresence>
        {' '}
        {/* Animate presence for the confirmation modal */}
            ...
            >
              ...
              <p className="text-gray-300 mb-6">
                ...
                <span className="font-bold text-yellow-400 mx-1">
                  {modalData?.amount} ETB
                </span>
                {modalData?.type === 'set'
                  ? 'as the TOTAL balance for'
                  : modalData?.type === 'add'
                    ? ' to'
                    : ' from'}{' '}
                user
                <span className="font-bold text-indigo-300 mx-1">
                  {modalData?.userId}
                </span>
                's wallet?
              </p>
          ...
          >
            ...
            >
              ...
              <p className="text-gray-300 mb-6 text-sm">
                This will permanently remove user{' '}
                <span className="font-bold text-red-400">{userToDelete}</span>.
                This action cannot be undone.
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setShowDeleteConfirm(false)}
                  className="flex-1 py-3 rounded-xl bg-white/10 text-white font-bold uppercase text-xs"
                >
                  Cancel
                </button>
                <button
                  onClick={handleDeleteUser}
                  className="flex-1 py-3 rounded-xl bg-red-600 text-white font-black uppercase text-xs"
                >
                  Delete User
                </button>
              </div>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex flex-col">
                  ...
                  <h3 className="text-xl font-black text-white italic truncate">
                    User:{' '}
                    {wallets[selectedUserActivity]?.username ||
                      selectedUserActivity}
                  </h3>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
                {activityLogs.length === 0 ? (
                  <div className="text-center py-10 text-gray-500 font-bold uppercase text-xs">
              ...
              <div className="grid grid-cols-5 gap-1.5 p-1 bg-black/20 rounded-2xl border border-white/5">
                {generateBoard(selectedBoard.id).map((row, rIdx) =>
                  row.map((cell: any, cIdx) => {
                    const isMarked =
                      cell.value === 'FREE' ||
                      selectedBoard.balls.includes(cell.value);
                    return (
                      <div
                        key={`${rIdx}-${cIdx}`}
  ...
  return (
    <div className={`${s.bg} ${s.border} border rounded-2xl p-4 shadow-md`}>
      <div className="flex items-center gap-2 mb-1">
        ...
        <span
          className={`text-[10px] font-black ${s.text} uppercase tracking-widest`}
        >
          {label}
        </span>
      </div>
      <div className="text-xl font-black text-white italic">
        ...
        {unit && (
          <span className={`text-xs not-italic ${s.icon} ml-1`}>{unit}</span>
        )}
      </div>
    </div>
  );
  ...
  return (
    <button
      onClick={() =>
        onSort({
          key: activeKey,
          order: isActive && currentSort.order === 'desc' ? 'asc' : 'desc',
        })
      }
